var newapp1=angular.module('studentregisterapp',['ngMaterial','ui.router','ui.bootstrap','ngSanitize','ngMessages']);
newapp1.controller('studentregistercntrl', ['$scope','studentRegisterService','$window','$location',function($scope,studentRegisterService,$window,$location) {
...
$scope.go=function(loginobj)
{
  ...
  studentRegisterService.insertDetails(loginobj).then(function(response)
    {
...
newapp1.service("studentRegisterService",function($http){



 this.insertDetails=function(loginobj)
{
      return $http.post("http://localhost:8083/"+"insertDetails",loginobj);
